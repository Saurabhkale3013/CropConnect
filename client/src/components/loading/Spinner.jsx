// import React from "react";

// function Spinner({color, width, marginRight="mr-2"}) {
//   return (
//     <svg
//       xmlns="http://www.w3.org/2000/svg"
//       viewBox="0 0 24 24"
//       fill="none"
//       className={`${width} animate-spin ${marginRight}`}
//     >
//       <path
//         opacity="0.2"
//         fillRule="evenodd"
//         clipRule="evenodd"
//         d="M12 19C15.866 19 19 15.866 19 12C19 8.13401 15.866 5 12 5C8.13401 5 5 8.13401 5 12C5 15.866 8.13401 19 12 19ZM12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"
//         fill={color}
//       />
//       <path
//         d="M2 12C2 6.47715 6.47715 2 12 2V5C8.13401 5 5 8.13401 5 12H2Z"
//         fill={color}
//       />
//     </svg>
//   );
// }

// export default Spinner;



import React from "react";
import PropTypes from "prop-types"; // ✨ Import PropTypes

function Spinner({ color, width, marginRight = "mr-2" }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="none"
      className={`${width} animate-spin ${marginRight}`}
    >
      <path
        opacity="0.2"
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12 19C15.866 19 19 15.866 19 12C19 8.13401 15.866 5 12 5C8.13401 5 5 8.13401 5 12C5 15.866 8.13401 19 12 19ZM12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"
        fill={color}
      />
      <path
        d="M2 12C2 6.47715 6.47715 2 12 2V5C8.13401 5 5 8.13401 5 12H2Z"
        fill={color}
      />
    </svg>
  );
}

// ✨ PropTypes added for validation
Spinner.propTypes = {
  color: PropTypes.string.isRequired,   // Ensures color is a required string
  width: PropTypes.string.isRequired,   // Ensures width is a required string (e.g., 'w-5', 'w-8')
  marginRight: PropTypes.string,        // Ensures marginRight is a string (optional, default value 'mr-2')
};

export default Spinner;
